...
/**
 * @brief Encrypts a data block using two AES-128 keys and combines the results with XOR.
 * 
 * @param block The input data block to be encrypted; modified with the encrypted result.
 * @param key A 32-byte key containing two AES-128 keys (16 bytes each).
 * @param nrounds Number of AES rounds to apply.
 * @param lastfull If true, includes MixColumn in the last round.
 */
void aes128_double_enc(uint8_t block[AES_BLOCK_SIZE], const uint8_t key[2 * AES_128_KEY_SIZE], unsigned nrounds, int lastfull);


/* Initialise aes structure for Q.2.2 */
struct aes128{
	uint8_t* s;
	uint8_t* sinv;
	uint8_t polynome;
...
extern struct aes128 AES128_PARAMS;
